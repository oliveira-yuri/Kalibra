...
import {
  ...
  Timer, TrendingDown, X, Zap, Moon, Sun, UploadCloud,
} from 'lucide-react';
...
function Edital({ workspaceSlug }: { workspaceSlug: string }) {
  const { user } = useUser();
  const { updateWorkspace } = useWorkspaces(user?.id);
  ...
  const saveEditalUpdate = () => {
    ...
    updateWorkspace(workspaceSlug, {
      ...
      nextAction: 'Edital atualizado · processamento aguardando backend',
    });
    setUpdateError('');
    setUpdateSaved(true);
    setTimeout(() => {
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 animate-in fade-in">
        <div className="k-card w-full max-w-md p-6">
          <div className="mb-4 flex items-center justify-between">
            ...
            <button className="k-icon-button k-button-quiet" onClick={() => setIsUpdateModalOpen(false)}><X size={16} /></button>
          </div>
          ...
          <div className="flex justify-end gap-3">
            ...
            <button className="k-button k-button-primary" onClick={saveEditalUpdate}>{updateSaved ? 'Edital recebido' : 'Salvar nova versão'}</button>
          </div>
        </div>
      </div>
    )}
